...
namespace map_renderer {
    ...
    void MapRenderer::SetRouteTextColorPproperties(svg::Text& text,
        const std::string& name,
        int palette,
        svg::Point position) const {
        SetRouteTextCommonProperties(text,
        ...
        for (auto [bus, palette] : buses_palette) {
            ...
            if (!bus_empty) {
                ...
                if (bus->is_roundtrip) {
                    ...
                    SetRouteTextColorPproperties(route_title_roundtrip,
                        std::string(bus->name),
                        palette,
                        sphere_projector(stops_geo_coords[0]));
                    ...
                    if (stops_geo_coords[0] != stops_geo_coords[stops_geo_coords.size() / 2]) {
                        ...
                        SetRouteTextColorPproperties(route_title_notroundtrip,
                            std::string(bus->name),
                            palette,
                            sphere_projector(stops_geo_coords[stops_geo_coords.size() / 2]));
